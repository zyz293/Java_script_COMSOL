...
public class Comsol3Dcode {
	...
	public Model run() throws Exception {
		...
		****/
		...
		savefile = "3D_comsolbuild_"+strACMode+"_"+ImageType+"_IP"+Double.toString(IP1)+"+"+Double.toString(IP2)+"_run_"+Double.toString(TrialNo);
			...
			if (isEpsDistribution == 1) { 
				if (ACMode == 1) {
					...
					for (int i = 0; i < NumOfTerms-1; i++) {
						...
						// System.out.println(MTD_Coeff[2*i]);
						// System.out.println(MTD_Coeff[2*i+1]);
...
/******************model.myfun_comsol_create_structure();************************/			
			...
			model.physics().create("ec", "ConductiveMedia", "geom1"); 
		        ...
		        model.physics("ec").feature("pc1").selection().set(new int[]{1, 6});/////////set bondouary!!!!!!!!!!!!!!!!!!!!
			...
			model.geom("geom1").run(); // current to run
			...
			model.geom("geom1").run(); // runCurrent to run
...
/***********************model.myfun_comsol_create_material();*********************/
			...
			if (isGetSolution == 1) {
				...
				if (ACMode == 1) {
					...
					String txtfilenameImag = savefile + "_CompPermImag.txt"; ///change
					...
					String txtfilenameReal = savefile + "_CompPermReal.txt"; // change
					...
					/***
					String Imagimage = savefile+"_CompPermImag.bmp";
					String Realimage = savefile+"_CompPermReal.bmp";
					model.result().export().create("imagplot","Image1D");
					model.result().export("imagplot").set("plotgroup","pg2");
					model.result().export("imagplot").set("bmpfilename","Imagimage");
					// model.result().export("imagplot").run();
					model.result().export().create("realplot","Image1D");
					model.result().export("realplot").set("plotgroup","pg3");
					model.result().export("realplot").set("bmpfilename","Realimage");
					// model.result().export("realplot").run();
					*///
