...
public class Comsol3Dcode {
	...
	public static void main(String[] args) throws Exception{\
		try {
			...
			new Comsol3Dcode().run(TrialNo, binsource, constt);
		} catch (Exception e) {}
	} 
	
	public Model run(double TrialNo, String binsource, Constt constt) throws Exception {
		ImageType = "recon";
		ControlMode = "linux";
	...
	/***********myfun_comsol_build(String PScoeff, String binsource, String savefile) throws Exception{**************/
		try {
			...
			if (isEpsDistribution == 1) {  
				...
				if (ACMode == 1) {
					...
					double[] MTD_Coeff = new double[2* ((int) NumOfTerms)-1];
					for (int i = 0; i < NumOfTerms-1; i++) {
						MTD_Coeff[2*i] = TemPR[i][2];
						MTD_Coeff[2*i+1] = TemPR[i][1];
					}
					MTD_Coeff[2*((int) NumOfTerms)-2] = TemPR[((int) NumOfTerms)-1][2] + extra_E_infi;
					...
					double NumOfTerms = Math.floor(MTD_Coeff.length / 2.0);
					String str_real;
					String str_imag;
					String str_Intreal;
					String str_Intimag;
